...
To run an archive node you must first reveiw the [full node](/join-the-network/full-node) setup. You will complete all the same steps. The only difference is you will add `--pruning archive` to the paramaters. 
